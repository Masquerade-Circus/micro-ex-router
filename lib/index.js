...
let addPath = (router, method, args) => {
  ...
  if (
    ...
    for (; i < l; i++) {
      ...
      router = addPath(router, subrouter.paths[i].method, submiddlewares);
    }
  }
 ...
 * @param  {Object}     opt // Additional options to parse the body
 * @return {Void} void
 */
  ...
  if (req.method.toLowerCase() !== 'get' && opt.parseBody) {
    ...
    try {
      if (type.includes('application/json')) {
        parsed = true;
        req.body = await micro.json(req, options);
      }

      if (type.includes('application/x-www-form-urlencoded') && !parsed) {
        parsed = true;
        req.body = await urlencodedBodyParser(req, options);
      }

      if (/^text/gi.test(type) && !parsed) {
        parsed = true;
        req.body = await micro.text(req, options);
      }
    } catch (e) {}
  }
};

let getMiddlewaresAndParams = (Router, req) => {
  ...
  for (; i < l; i++) {
    ...
    if (
      (path.method === 'use' || method === path.method) &&
      path.path === undefined
    ) {
...
let RouterFactory = (options = {}) => {
  let opt = Object.assign(
    {},
    {
      ...
      // How much data is aggregated before parsing at max. It can be a Number of bytes or a string like '1mb'.
      limit: '1mb',
      encoding: 'utf8',
      acceptedMethods: [
        'get',
  ...
  let Router = async function (req, res) {
    ...
    let { middlewares, params } = getMiddlewaresAndParams(Router, req);
    ...
    if (!res.headersSent) {
      throw new Error(
        `The url ${req.url} requested by ${method}, wasn't found`
      );
    }
  };
...
RouterFactory.serveDir = serve.serveDir;
RouterFactory.serveFile = serve.serveFile;
RouterFactory.render = serve.render;
